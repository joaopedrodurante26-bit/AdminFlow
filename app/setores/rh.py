...
def abrir_registrar_ferias(root):
    janela = tk.Toplevel(root)
    janela.title("Registrar Férias")
    janela.geometry("760x650")
    janela.minsize(700, 580)
    ...
    status = tk.StringVar(value="Programada")
    ...
    for campo_data in campos_datas:
        campos[campo_data].bind(
            "<KeyRelease>",
            lambda evento, nome=campo_data: aplicar_mascara_data(nome)
        )
    ...
    def selecionar_documento():
        arquivo = filedialog.askopenfilename(
            title="Selecione documento de férias",
            filetypes=[
                ("PDF e imagens", "*.pdf *.png *.jpg *.jpeg"),
                ("Todos os arquivos", "*.*")
            ]
    ...
    def exibir_resumo_confirmacao(dados):
        janela_resumo = tk.Toplevel(janela)
        janela_resumo.title("Confirmar Registro de Férias")
        janela_resumo.geometry("680x500")
        janela_resumo.minsize(620, 450)

        tk.Label(
            janela_resumo,
            text="Resumo do Registro de Férias",
            font=("Arial", 16, "bold")
        ).pack(pady=15)
        ...
        caixa_texto.insert(tk.END, "DADOS DAS FÉRIAS\n")
        caixa_texto.insert(tk.END, "=" * 45 + "\n\n")
    ...
    def executar_registro(dados):
        try:
            registrar_ferias(dados, documento_anexado["caminho"])
            ...
            messagebox.showerror(
                "Erro inesperado",
                f"Ocorreu um erro ao registrar as férias:\n\n{erro}"
            )

    def finalizar():
        dados = coletar_dados()

        valido, mensagem = validar_dados_ferias(
            dados,
            documento_anexado["caminho"]
        )
...
def abrir_registrar_advertencia(root):
    janela = tk.Toplevel(root)
    janela.title("Registrar Advertência")
    janela.geometry("780x680")
    janela.minsize(720, 600)
    ...
    tk.Label(frame_form, text="Tipo de advertência:").grid(
        row=linha,
        column=0,
        sticky="e",
        padx=10,
    ...
    tipo_advertencia = tk.StringVar(value="Selecione")
    tk.OptionMenu(frame_form, tipo_advertencia, *TIPOS_ADVERTENCIA).grid(
        row=linha,
        column=1,
        sticky="w",
        padx=10,
    ...
    campos["Tipo de advertência"] = tipo_advertencia

    linha += 1

    tk.Label(frame_form, text="Gravidade:").grid(
        row=linha,
        column=0,
        sticky="e",
        padx=10,
    ...
    gravidade = tk.StringVar(value="Selecione")
    tk.OptionMenu(frame_form, gravidade, *GRAVIDADES).grid(
        row=linha,
        column=1,
        sticky="w",
        padx=10,
    ...
    campos["Gravidade"] = gravidade

    def aplicar_mascara_data(evento):
        texto = campos["Data da ocorrência"].get()
        numeros = "".join(c for c in texto if c.isdigit())[:8]
        ...
        campos["Data da ocorrência"].delete(0, tk.END)
        campos["Data da ocorrência"].insert(0, formatado)

    campos["Data da ocorrência"].bind("<KeyRelease>", aplicar_mascara_data)
    ...
    def selecionar_documento():
        arquivo = filedialog.askopenfilename(
            title="Selecione documento da advertência",
            filetypes=[
                ("PDF e imagens", "*.pdf *.png *.jpg *.jpeg"),
                ("Todos os arquivos", "*.*")
            ]
    ...
    def exibir_resumo_confirmacao(dados):
        janela_resumo = tk.Toplevel(janela)
        janela_resumo.title("Confirmar Advertência")
        janela_resumo.geometry("700x520")
        janela_resumo.minsize(640, 460)

        tk.Label(
            janela_resumo,
            text="Resumo da Advertência",
            font=("Arial", 16, "bold")
        ).pack(pady=15)
        ...
        caixa_texto.insert(tk.END, "DADOS DA ADVERTÊNCIA\n")
        caixa_texto.insert(tk.END, "=" * 45 + "\n\n")
    ...
    def executar_registro(dados):
        try:
            registrar_advertencia(dados, documento_anexado["caminho"])
            ...
            messagebox.showerror(
                "Erro inesperado",
                f"Ocorreu um erro ao registrar a advertência:\n\n{erro}"
            )

    def finalizar():
        dados = coletar_dados()

        valido, mensagem = validar_dados_advertencia(
            dados,
            documento_anexado["caminho"]
        )
