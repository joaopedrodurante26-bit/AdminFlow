...
def abrir_arquivar_nf_recebida(root):
    janela = tk.Toplevel(root)
    janela.title("Arquivar NF Recebida")
    janela.geometry("780x640")
    janela.minsize(720, 580)
    ...
    def selecionar_nf():
        arquivo = filedialog.askopenfilename(
            title="Selecione o arquivo da NF recebida",
            filetypes=[
                ("PDF e XML", "*.pdf *.xml"),
                ("Todos os arquivos", "*.*")
            ]
    ...
    def exibir_resumo_confirmacao(dados):
        janela_resumo = tk.Toplevel(janela)
        janela_resumo.title("Confirmar NF Recebida")
        janela_resumo.geometry("700x500")
        janela_resumo.minsize(640, 450)

        tk.Label(
            janela_resumo,
            text="Resumo da NF Recebida",
            font=("Arial", 16, "bold")
        ).pack(pady=15)
        ...
        caixa_texto.insert(tk.END, "DADOS DA NF RECEBIDA\n")
        caixa_texto.insert(tk.END, "=" * 45 + "\n\n")
    ...
    def executar_arquivamento(dados):
        try:
            arquivar_nf_recebida(dados, arquivo_nf["caminho"])

            messagebox.showinfo(
                "Arquivamento concluído",
                "NF recebida arquivada com sucesso."
            )
            ...
            messagebox.showerror(
                "Erro inesperado",
                f"Ocorreu um erro ao arquivar a NF recebida:\n\n{erro}"
            )

    def finalizar():
        dados = coletar_dados()

        valido, mensagem = validar_dados_nf_recebida(
            dados,
            arquivo_nf["caminho"]
        )
